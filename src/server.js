...
module.exports = async (client) => {
        ...
        if (banned) {
            client.users.fetch(req.user.id).then(async a => {
                client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({
                    ...
                    dynamic: true
                })).setColor("RED").setDescription(`[**${a.username}**#${a.discriminator}](https://dislist.yghostbr.repl.co/user/${a.id}) logged into the site.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
            })
            req.session.destroy(() => {
                res.json({
                    login: false,
            ...
            client.users.fetch(req.user.id).then(async a => {
                client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({
                    ...
                    dynamic: true
                })).setColor("GREEN").setDescription(`[**${a.username}**#${a.discriminator}](https://dislist.yghostbr.repl.co/user/${a.id}) Just Logged into the site.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
    ...
    app.get("/dc", (req, res) => {
        res.redirect('https://discord.gg/NDtJnZKFVe');
    });
    ...
    app.get("/robots.txt", function(req, res) {
        ...
        res.send(`Sitemap: https://dislist.yghostbr.repl.co/sitemap.xml`);
    });

    app.get("/sitemap.xml", async function(req, res) {
        let link = "<url><loc>https://dislist.yghostbr.repl.co/</loc></url>";
        let botdataforxml = await botsdata.find()
        botdataforxml.forEach(bot => {
            link += "\n<url><loc>https://dislist.yghostbr.repl.co/bot/" + bot.botID + "</loc></url>";
        })
    ...
    app.post("/addbot", checkMaintence, checkAuth, async (req, res) => {
        ...
        client.users.fetch(rBody['botID']).then(a => {
            client.channels.cache.get(channels.botlog).send(`<:addbot:896201320246173746> <@${req.user.id}> added **${a.tag}**`)
    ...
    app.get("/admin/confirm/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.users.fetch(req.params.botID).then(bota => {
            client.channels.cache.get(channels.botlog).send(`<:addbot:896201320246173746> <@${botdata.ownerID}>'s bot named **${bota.tag}** has been approved. `)
    ...
    app.post("/admin/decline/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.users.fetch(botdata.ownerID).then(sahip => {
            client.channels.cache.get(channels.botlog).send(`<:removebot:896201428635353109> <@${botdata.ownerID}>'s bot named **${botdata.username}** has been declined. `)
            client.users.cache.get(botdata.ownerID).send(`<:removebot:896201428635353109> Your bot named **${botdata.username}** has been declined.\nReason: **${rBody['reason']}**\nAuthorized: **${req.user.username}**`)
        })
        await botsdata.deleteOne({
            botID: req.params.botID,
            ownerID: botdata.ownerID
    ...
    app.post("/admin/certificate/delete/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.users.fetch(botdata.botID).then(bota => {
            ...
            client.users.cache.get(botdata.ownerID).send(`<:removebot:896201428635353109> Your bot named **${bota.tag}** certificate application has been declined.\nReason: **${rBody['reason']}**`)
        });
        await appsdata.deleteOne({
            botID: req.params.botID
        })
    ...
    app.post("/admin/addcode", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.channels.cache.get(channels.codelog).send(new Discord.MessageEmbed()
            ...
            .setDescription(`The user named **[${req.user.username}](https://dislist.yghostbr.repl.co/user/${req.user.id})** added the code named **${rBody['codename']}** to the system.`)
            .addField("Code Link", `https://dislist.yghostbr.repl.co/code/${kod}`, true)
    ...
    app.post("/admin/editcode/:code", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.channels.cache.get(channels.codelog).send(new Discord.MessageEmbed()
            ...
            .setDescription(`The user named **[${req.user.username}](https://dislist.yghostbr.repl.co/user/${req.user.id})** edited the code named **${rBody['codename']}**.`)
            .addField("Code Link", `https://dislist.yghostbr.repl.co/code/${kod}`, true)
            .addField("Code Description", rBody['codedesc'], true)
            .addField("Code Category", rBody['category'], true)
        )
    ...
    app.post("/admin/maintence", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.channels.cache.get(channels.webstatus).send(`Topic has been switched to __maintance__ due to **${req.body.reason}**`).then(a => {
            new maintenceSchema({
                server: config.server.id,
    ...
    app.post("/admin/unmaintence", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        const bakimsonaerdikardesTopic = new dc.MessageEmbed()
            .setAuthor("Dis LIST", client.user.avatarURL())
            .setThumbnail(client.user.avatarURL())
            .setColor("GREEN")
            .setDescription(`<:bugchecker:896197924445364224> Topic are **active** again!\n[Click to redirect website](https://dislist.yghostbr.repl.co)`)
            .setFooter("Topic © All rights reserved.");
        await client.channels.cache.get(channels.webstatus).messages.fetch(bakimdata.bakimmsg).then(a => {
            a.edit(`~~ <:bugchecker:896197924445364224> Topic has been switched to __maintance__ due to **${bakimdata.reason}** ~~`, bakimsonaerdikardesTopic)
        })
